...
def rot_matrix_2D(rot, deg=True):
    if deg:
        rot = np.deg2rad(rot)
...
def polar_to_cartesian(polar_coords):
    r = polar_coords[:, 0]
    theta = polar_coords[:, 1]  # azimuthal angle (-180 to +180)
    x = r * np.cos(theta)
    y = r * np.sin(theta) + d / 2
    cartesian_coords = np.vstack((x, y)).T
    return cartesian_coords
...
class PointScatterer:
    ...
    def __init__(self, RCS_dB, x, y):
        self.RCS = 10 ** (RCS_dB / 10)
        self.pos = np.array([x, y])

    def backscatter(self, source, recpos, rot, deg=True):
        pos = np.matmul(rot_matrix_2D(rot, deg), self.pos)
...
def find_best_fit_phase_center(rot_angles, phase_center):
    unrotated_ph_c = np.array([np.matmul(rot_matrix_2D(-rot, True), phase_center[i, :])
                               for i, rot in enumerate(rot_angles)])
    ph_c_fit = np.average(unrotated_ph_c, axis=0)
    fitted_phase_center = np.array([np.matmul(rot_matrix_2D(rot, True), ph_c_fit)
...
for i, rot in enumerate(rot_angles):
    ...
    for ps in point_scatterers:
        rec += np.array([ps.backscatter(source, r_pos, rot, deg=True) for r_pos in rec_poses])
    received[i, :] = rec
...
phase_center_sph = np.array((-np.unwrap(np.angle(received[:, 0]) + zero_phase) / source.k / 2 + r,
                             np.arcsin(np.unwrap(np.angle(received[:, 1]) - np.angle(received[:, 0])) / source.k / d))).T

phase_center = polar_to_cartesian(phase_center_sph) + np.array((-r, 0))
