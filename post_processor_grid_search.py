...
from functions import spherical_to_cartesian, process_file, get_grd_files_grid, unrotate_spherical, \
    fft_resample_complex_with_padding, rotate_point, unwrap_2d_branch_cut
from scipy.interpolate import RegularGridInterpolator

# Constants
d = 1.5/100  # 1.5 cm receiver spacing
...
def process_folder_grid(base_path):
    ...
    for grd in grd_paths:
        ...
        sig.append(process_file(grd, mask=[-5, -4, -2], polarisation=pol))  # use for 3*3 grid
...
folder_path = r"C:\Users\titiv\OneDrive - Danmarks Tekniske Universitet\DTU\Thesis\sim_results\Try_horizontal_6cm_spacing"
...
# folder_path = r"C:\Users\titiv\OneDrive - Danmarks Tekniske Universitet\DTU\Thesis\sim_results\ugly_ducling_80_100_85_95_6cm_spacing"
...
# r_ = -np.unwrap(np.unwrap(np.angle(received[:, :, 0]) + zero_phase, axis=1), axis=0)/k/2 + r - radial_offset
...
#                                       (2 * np.pi) - np.pi), axis=1), axis=0) / k / d)
# amb_ang = np.arcsin(2*np.pi / k / d)
# theta_ = theta_ - amb_ang * (theta_ > 0.01)
# psi_ = psi_ + amb_ang * (psi_ < -amb_ang/2)
# psi_ = psi_ - amb_ang * (psi_ > amb_ang/2)

# r_ = -unwrap2D(np.angle(received[:, :, 0]) + zero_phase)/k/2 + r - radial_offset
r_ = -unwrap_2d_branch_cut(np.angle(received[:, :, 0]) + zero_phase)/k/2 + r - radial_offset
# r_ = -unwrap(np.angle(received[:, :, 0]) + zero_phase)/k/2 + r - radial_offset
theta_ = np.arcsin(unwrap_2d_branch_cut((np.angle(received[:, :, 1]) - np.angle(received[:, :, 0]) + np.pi) %
                          (2 * np.pi) - np.pi) / k / d)
psi_ = np.arcsin(unwrap_2d_branch_cut((np.angle(received[:, :, 2]) - np.angle(received[:, :, 0]) + np.pi) %
                        (2 * np.pi) - np.pi) / k / d)
...
best_guess_ps = np.array((np.mean(x), np.mean(y), np.mean(z)))
print(f"Best guess for phase center: {np.mean(x)}, {np.mean(y)}, {np.mean(z)}")
...
poly_collection.set_color((0.7, 0.2, 0.2, 0.0))
poly_collection.set_edgecolor((0.7, 0.2, 0.2, 0.1))
ax.add_collection3d(poly_collection)
# Plot the point cloud
sc = ax.scatter(x_, y_, z_, c=color_values_flat, cmap='viridis', marker='.', alpha=1)
# Add a color bar
...
rotated_best_guess = rotate_point(best_guess_ps, theta, psi)
fig, axes = plt.subplots(1, 4, figsize=(10, 3))

# Plot x coordinates (delta r)
c1 = axes[1].imshow((phase_center_sph[:, :, 0] - r - rotated_best_guess[:, :, 0]) * 100,
                    extent=[psi.min(), psi.max(), theta.max(), theta.min()],
                    origin='upper', aspect='equal', cmap='viridis')
...
# Plot y coordinates
c3 = axes[2].imshow((phase_center[:, :, 1] - rotated_best_guess[:, :, 1]) * 100,
                    extent=[psi.min(), psi.max(), theta.max(), theta.min()],
                    origin='upper', aspect='equal', cmap='viridis')
axes[2].set_title(r"$\Delta y$")
axes[2].set_ylabel('Theta [deg]', labelpad=-0)
axes[2].set_xlabel('Psi [deg]')
fig.colorbar(c3, ax=axes[2], label='[cm]')

# Plot z coordinates
c4 = axes[3].imshow((phase_center[:, :, 2] - rotated_best_guess[:, :, 2]) * 100,
                    extent=[psi.min(), psi.max(), theta.max(), theta.min()],
                    origin='upper', aspect='equal', cmap='viridis')
axes[3].set_title(r"$\Delta z$")
